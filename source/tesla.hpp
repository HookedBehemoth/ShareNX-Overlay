...
public:
	...
	virtual std::pair<u16, u16> getPosition() { return {this->m_x, this->m_y}; };
	virtual std::pair<u16, u16> getSize() { return {this->m_width, this->m_height}; };
...
public:
	...
	virtual void draw(gfx::Renderer *renderer) override {
		...
		for (auto &entry : this->m_items) {
			i++;
			if (i < this->m_offset || i > this->m_entriesShown + this->m_offset)
				continue;
